...
- ["Дозапросы"](#дозапросы)
  ...
  - [Подсистема "Дозапросы"](#подсистема-дозапросы)
    ...
    - [Ручной режим отправки дозапросов](#ручной-режим-отправки-дозапросов)
      - [Пример ручной отправки дозапроса в систему УНИКУМ](#пример-ручной-отправки-дозапроса-в-систему-уникум)
...
- Отложенный. Дозапросы и сообщения Datareon ESB записываются в промежуточные хранилища ["Объекты подсистемы"](#объекты-подсистемы).
Включение отложенного режима регулируется **константой "Использовать отложенные дозапросы"** с использованием метода [ЗаписатьОтложенныеДозапросы](#записатьотложенныедозапросы) во входящем обработчике.
...
**Отправка дозапроса выполняется по классу Request[32] Datareon ESB.**
...
- **Константа "Использовать исключения для не найденных объектов дозапросы"** - вызывает в обработчике исключения при наличии ненайденных объектов, сообщение переходит в статус "Ошибка обработки"
- **Служебный обработчик [Идентификаторы потоков ESB](ИдентификаторыПотоковESB.bsl)** - Исходящий служебный обработчик Datareon ESB c описанием настроек интеграциооных потоков idflow.
- **[Исходящий обработчик "Request.Out"](Request.Out.bsl)** - Исходящий обработчик Datareon ESB , формирует xml пакеты дозапросов.
- **Регламентное задание "Обработать пакеты"** - задание на повторную обработку сообщения в регистре сведений "Источники дозапросов" по которым пришли все дозапросы.
...
- [**Регистр сведений "Источники дозапросов"**][регистрсведенийисточникидозапросов] - хранилище дозапросов.
  
  ![Источники дозапросов](ИсточникиДозапросов.png)

### Ручной режим отправки дозапросов

При работе интеграции может возникать необходимость отправки ручных дозапросов для оператичного получения информации из внешних систем, проверки работы интеграционных потоков.
Для этих целей предусмотрен ручной режим отправки дозапросов.

Общая схема отправки ручных дозапросов.

1. Получение уникального идентификатора внешней системы.
2. Определение idflow( для УНИКУМ достаточно определить интеграционный класс )
3. Настройка параметров отправки дозапросов
4. Обработка / отправка дозапроса внешней системой
5. Получение ответа на дозапрос.

#### Пример ручной отправки дозапроса в систему УНИКУМ

1. Получение уникального идентификатора внешней системы.
Для опредления идентификатора в системе ЕРП требуется в форме элемента перейти через навигационную панель в форму списка "Таблица переходный ключей ЕРП", скопировать в буфер обмена идентификатор.
Если идентфикатор не известен, требуется определить идентификатор в УНИКУМ, используя обработку "Просмотр ID.ERT".
![Определение идентификатора объекта дозапросов](РучнаяОтправкаДозапроса_ПоискИдентификатораОбъекта.png)
2. Определение idflow( для УНИКУМ достаточно определить интеграционный класс )
3. Настройка параметров отправки дозапросов
4. Обработка / отправка дозапроса внешней системой
5. Получение ответа на дозапрос.
