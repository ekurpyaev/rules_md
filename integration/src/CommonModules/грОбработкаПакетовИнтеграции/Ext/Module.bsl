...
//
Процедура УстановитьСостояниеСообщения(ИдентификаторСообщения, НовоеСостояние,
	Задержка = 0, ОписаниеОшибки = "", ИндексПотока = 0, ПроверкаОжиданий = Истина ) Экспорт
	...
	Если НовоеСостояние = Перечисления.сшпСтатусыСообщений.ОжиданиеОбработки И ПроверкаОжиданий Тогда
...
Функция ПолучитьИдентификаторОбъектаERP(ОбъектERP, ВидИдентификатораСтрока, ДополительныеПараметры = Неопределено) Экспорт
	...
	Если Не ПроверитьФорматТаблицыТипыПрефиксыКлючейESB( ТипыПрефиксыКлючейESB ) Тогда
	...
	Если ВидИдентификатораСтрока = "УНИКУМGUID" Тогда
		...
		Если МассивОтборов.Количество() > 0 Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&ОтборПоПрефиксам", " И" + ТекстОтбора);
		Иначе
			
			СтруктураВозврата.НеНайденОсновнойПрефикс = Истина;
		...
		Если Выборка.Следующий() Тогда
			...
			СтруктураВозврата.ИдентификаторОбъекта = Выборка.ИдентификаторОбъекта;
...
Функция ПолучитьСтруктуруТаблицыТипыПрефиксыУНИКУМ() Экспорт 
	
    ТаблицаТипыПрефиксыУНИКУМ = Новый ТаблицаЗначений;
	ТаблицаТипыПрефиксыУНИКУМ.Колонки.Добавить("ТипОбъекта",	ОбщегоНазначения.ОписаниеТипаСтрока(150));
	ТаблицаТипыПрефиксыУНИКУМ.Колонки.Добавить("Префикс",		ОбщегоНазначения.ОписаниеТипаСтрока(4));
	ТаблицаТипыПрефиксыУНИКУМ.Колонки.Добавить("ТипОбъектаЕРП",	ОбщегоНазначения.ОписаниеТипаСтрока(150));
	ТаблицаТипыПрефиксыУНИКУМ.Колонки.Добавить("Основной",		Новый ОписаниеТипов("Булево"));
	
	Возврат ТаблицаТипыПрефиксыУНИКУМ;
	
КонецФункции

Функция ПроверитьФорматТаблицыТипыПрефиксыКлючейESB( ТипыПрефиксыКлючейESB )
	
	ТипыПрефиксыКлючейESB_Эталон = грОбработкаПакетовИнтеграции.ПолучитьСтруктуруТаблицыТипыПрефиксыУНИКУМ();
	
	ПроверкаПройдена = Ложь;
	Если ЗначениеЗаполнено( ТипыПрефиксыКлючейESB ) 
		И Тип("ТаблицаЗначений") =  ТипЗнч( ТипыПрефиксыКлючейESB )Тогда
				
		КолонкиЭталон = Новый Массив;
		Для Каждого Колонка Из ТипыПрефиксыКлючейESB_Эталон.Колонки Цикл
			КолонкиЭталон.Добавить(Колонка.Имя);
		КонецЦикла;
		
		КолонкиТаблицы = Новый Массив;
		Для Каждого Колонка Из ТипыПрефиксыКлючейESB.Колонки Цикл
			КолонкиТаблицы.Добавить(Колонка.Имя);
		КонецЦикла;
	
		ПроверкаПройдена = ОбщегоНазначения.КоллекцииИдентичны( КолонкиЭталон, КолонкиТаблицы ); 
