...
Функция ВыполнитьОбновлениеДанныхКонфигурации() Экспорт
	...
	ИначеЕсли ТекущаяВерсия <= 10309 Тогда
			 	
		//Проверка необходимости делать переход на 1.4.0 с обработкой 
		Если ОбновлениеВерсии10400() Тогда
		 	
			НоваяВерсия = 10401; // Версия 1.4.1
		
		Иначе
			
			НоваяВерсия = 10309; // Версия 1.3.9
		
		КонецЕсли;
		
	ИначеЕсли ТекущаяВерсия <= 10401 Тогда
		
		Если ОбновлениеВерсии10403() Тогда
			НоваяВерсия = 10403;
		КонецЕсли;
		
	ИначеЕсли ТекущаяВерсия <= 10403 Тогда
		
		Если ОбновлениеВерсии10404() Тогда
			НоваяВерсия = 10404;
		КонецЕсли;

	ИначеЕсли ТекущаяВерсия <= 10404 Тогда
